...
CREATE TABLE Kadri_online_access_period (
    ...
    CONSTRAINT for_user FOREIGN KEY (user_login) REFERENCES Users(user_login),
    CONSTRAINT fk_tax_plan FOREIGN KEY (tax_plan_ID) REFERENCES Kadri_online_tax_plan(tax_plan_ID)
);
...
CREATE TABLE Kadri_online_settings (
    ...
    date_advance INT CHECK (date_advance BETWEEN 1 AND 28), -- Ограничение на диапазон
    date_salary INT CHECK (date_salary BETWEEN 1 AND 28),
    reserv_money BOOLEAN,
    month_off INT CHECK (month_off BETWEEN 1 AND 28),
    date_send_pers INT CHECK (date_send_pers BETWEEN 1 AND 28),
    date_send_6_ndfl INT CHECK (date_send_6_ndfl BETWEEN 1 AND 28),
    date_send_efc_1 INT CHECK (date_send_efc_1 BETWEEN 1 AND 28),
    date_send_rsv INT CHECK (date_send_rsv BETWEEN 1 AND 28),
    CONSTRAINT for_user FOREIGN KEY (user_login) REFERENCES Users(user_login)
);
...
INSERT INTO Kadri_online_access_period (user_login, tax_plan_ID, access_period_start) VALUES
    ('loginDD', 1, '2024-10-23'), -- Используем существующий tax_plan_ID (например, 1_year)
    ('loginNN', 2, '2024-11-01'); -- Используем существующий tax_plan_ID (например, 30_days)
...
VALUES ('loginDD', 2024, 25, 10, true, 5, 7, 7, 8, 8);
...
VALUES ('loginNN', 2024, 25, 10, false, 5, 7, 7);
