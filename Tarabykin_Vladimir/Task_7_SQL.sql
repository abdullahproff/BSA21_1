...
CREATE TABLE Counterparty (
    ...
    mcc INTEGER                -- MCC-код (может быть NULL для некоторых типов)
);
...
LEFT JOIN Category c ON cm.category_name = c.name
...
LEFT JOIN Counterparty c ON cn.counterparty_title = c.title
LEFT JOIN Category_MCC cm ON t.mcc = cm.mcc
LEFT JOIN Category cat ON cm.category_name = cat.name
